...
public Component create(Document pdfDoc, Component c) throws IOException{
    ...
    tableRow.add(tabel,5, "RECTANGLE",                     c.componentID  );
    ...
    tableRow.add(tabel,0, "Visible",      c.visible,      "Fill Color",   c.fillcolor  );
    tableRow.add(tabel,0, "Mode",         c.mode,         "Fill" ,        c.doFill     );
    tableRow.add(tabel,0, "Border color", c.bordercolor,  "Font",         c.font       );
    ...
    if (check.isTag(c.tagSW))      { VocamAnalyser.reportTags.addTag("Rectangle","TagSW", c.tagSW, c.componentID); }
    if (check.isTag(c.show))       { VocamAnalyser.reportTags.addTag("Rectangle","Show", c.show,  c.componentID); }
